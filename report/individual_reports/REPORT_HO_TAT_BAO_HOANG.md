...
4.  **Observed Failures (Các lỗi quan sát được)**:
    - **Ảo tưởng (Hallucination)**: LLM tự động bịa ra thông tin học viên hư cấu hoặc tự dự đoán kết quả công cụ (Observation) khi parser hoạt động không chuẩn xác hoặc khi công cụ trả về định dạng tham số sai/lệch so với kỳ vọng của mô hình.
    - **Vượt rào bảo mật (Jailbreak/Prompt Injection)**: Kẻ tấn công có thể dễ dàng chèn câu lệnh độc hại (như "ignore previous instructions") để vượt qua các bộ lọc từ khóa đơn giản, làm chệch hướng lập luận hoặc ép Agent tiết lộ cấu hình hệ thống nếu Guardrail chưa được thiết lập sâu bằng LLM.
    - **Vòng lặp vô hạn (Infinite Loop)**: LLM gọi liên tiếp một công cụ chẩn đoán với cùng tham số do không tìm thấy điều kiện dừng hoặc parser bị lỗi, gây tốn tài nguyên và tăng chi phí ngoài tầm kiểm soát.
